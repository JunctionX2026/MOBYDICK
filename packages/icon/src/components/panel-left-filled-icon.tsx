import { Icon } from "../icon";
import type { IconProps } from "../types";

/** Generated from the Tabler filled icon "layout-sidebar". Run `pnpm generate` to update. */
export const PanelLeftFilledIcon = (props: IconProps) => (
  <Icon {...props}>
    <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
      <path d="M6 21a3 3 0 0 1 -3 -3v-12a3 3 0 0 1 3 -3h12a3 3 0 0 1 3 3v12a3 3 0 0 1 -3 3zm12 -16h-8v14h8a1 1 0 0 0 1 -1v-12a1 1 0 0 0 -1 -1" />
    </svg>
  </Icon>
);

PanelLeftFilledIcon.displayName = "PanelLeftFilledIcon";
